    ...
    public void run() throws Exception {
        ...
        eventloop: while (!this.stop) {
            ...
            case "init":
            	...
            	final Float timeResolution;
            	if (req.args.size() > 1) {
            		timeResolution = (Float) req.args.get(1);
                } 
            	else {
            		timeResolution = (float) 1;
                	System.out.println("No time resolution was sent by mosaik. Using standard value of 1.0 (seconds).");
                }
                result = this.sim.init(sid, timeResolution, req.kwargs);
                break;
            ...
            case "step":
                ...
                final long maxAdvance;
                if (req.args.size() > 2) {
                	maxAdvance = ((Number) req.args.get(2)).longValue();
                } else {
                	maxAdvance = time;
                	System.out.println("No maxAdvance was sent by mosaik. Using time instead: " + time);
                }
                result = this.sim.step(time, inputs, maxAdvance);
                break;
